...
class VoxelSetAbstraction(nn.Module):
    ...
    def get_sampled_points(self, batch_dict):
        ...
        if self.model_cfg.POINT_SOURCE == 'raw_points':
            ...
            # print("src_points_size",src_points.size())
        ...
        for bs_idx in range(batch_size):
            ...
            if self.model_cfg.SAMPLE_METHOD == 'FPS':
                ...
                # print("cur_pt_indxs",cur_pt_idxs)
                # print("sample_point_size",sampled_points.size())
                ...
                # if sampled_points.shape[1] < self.model_cfg.NUM_KEYPOINTS:
                #     empty_num = self.model_cfg.NUM_KEYPOINTS - sampled_points.shape[1]
                #     if empty_num <= sampled_points.shape[1]:
                #         cur_pt_idxs[0, -empty_num:] = cur_pt_idxs[0, :empty_num]
                        
                #     else:
                #         # Pad cur_pt_idxs multiple times to reach the desired length
                #         if sampled_points.shape[1] > 0:
                #             # print("too many empty")    
                #             num_pads = empty_num // sampled_points.shape[1] + 1
                #             padded_idxs = torch.cat([cur_pt_idxs] * num_pads, dim=1)
                #             cur_pt_idxs[0, -empty_num:] = padded_idxs[0, :empty_num]
                #         else:
                #             print("sampled_points is 0", sampled_points.shape[1])
                #             # print(cur_pt_idxs)
                #             # print(src_points)
                #             # s_mask = (batch_indices == bs_idx-1)
                #             # sampled_points = src_points[bs_mask].unsqueeze(dim=0)
                #             # print("sampled_points_last_frame", sampled_points)
                #             # print("size",sampled_points.size())
                #             # sampled_points[:] = 0
                #             # print("sampled_points_after_o",sampled_points)
                #             # print("size",sampled_points.size())
                #             sampled_points = src_points
                #             sampled_points = sampled_points.unsqueeze(0)
                #             num_pads = empty_num // (sampled_points.shape[1] + 1) + 1
                #             padded_idxs = torch.cat([cur_pt_idxs] * num_pads, dim=1)
                #             cur_pt_idxs[0, -empty_num:] = padded_idxs[0, :empty_num]
                ...
                # print("keypoints_size",keypoints.size())
